...
function SectionKicker({ number, label, light = false }: { number: string; label: string; light?: boolean }) {
  return (
    <div className={`flex items-center gap-3 ${light ? "text-white/60" : "text-[#777267]"}`}>
      <span className="tf-mono text-[10px]">{number}</span>
      <span className={`h-px w-8 ${light ? "bg-white/30" : "bg-[#2b2925]/25"}`} />
...
function AppMark({ dark = false }: { dark?: boolean }) {
  return (
    <a href="#top" className="tf-focus flex items-center gap-2" aria-label="TerraForge Robotics home">
      ...
      <span className={`text-[11px] font-semibold tracking-[.14em] ${dark ? "text-white" : "text-[#2b2925]"}`}>TERRAFORGE</span>
    </a>
  );
}
...
export default function Home() {
  ...
  return (
    <div id="top" className="tf-page">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/15 bg-[#1f221f]/30 text-white backdrop-blur-[10px]">
        <div className="tf-container flex h-[72px] items-center justify-between">
          <AppMark dark />
          <nav className="hidden items-center gap-8 md:flex" aria-label="Primary navigation">
            {navItems.map((item, index) => (
              <a key={item.href} href={item.href} className={`tf-focus relative text-[11px] font-medium text-white/75 transition-colors hover:text-white ${index === 0 ? "text-white after:absolute after:-bottom-[27px] after:left-0 after:h-[2px] after:w-full after:bg-[#e85b2a]" : ""}`}>
                {item.label}
              </a>
            ))}
          </nav>
        ...
        {menuOpen && (
          <div className="border-t border-white/15 bg-[#252724] px-6 py-5 md:hidden">
            <div className="flex flex-col gap-4">
              {navItems.map((item) => (
                <a key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className="tf-mono text-[11px] text-white/80 hover:text-[#e85b2a]">{item.label}</a>
              ))}
      ...
      <main>
        <section className="tf-scanline relative min-h-[720px] overflow-hidden bg-[#4b392d] text-white sm:min-h-[860px] lg:min-h-[100svh]" aria-label="TerraForge Robotics introduction">
          ...
          <div className="tf-container relative flex min-h-[720px] items-end pb-12 pt-32 sm:min-h-[860px] sm:pb-20 lg:min-h-[100svh] lg:pb-16">
            <div className="max-w-[560px]">
              <div className="reveal flex items-center gap-3 text-white/70">
                <span className="h-2 w-2 rounded-full bg-[#88bc62] shadow-[0_0_0_5px_rgba(136,188,98,.18)]" />
                <span className="tf-mono text-[10px]">Field system / online</span>
              </div>
              <h1 className="hero-copy reveal reveal-delay-1 mt-5 max-w-[620px] text-[clamp(3rem,9vw,7.5rem)] font-medium leading-[.9] tracking-[-.075em] text-white">More acres.<br /><span className="text-[#f6b27c]">Fewer</span> compromises.</h1>
        ...
        <section className="tf-surface border-b border-[#2b2925]/15 py-16 sm:py-20" id="machine">
          <div className="tf-container">
            <div className="grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:gap-20">
              <div>
                ...
                <p className="mt-6 max-w-[390px] text-base leading-7 text-[#5f5b53]">TerraForge makes farm work more repeatable. One platform, a set of tools, and a calmer way to keep every pass on schedule.</p>
                <div className="mt-8 flex flex-wrap gap-2">
                  {(Object.keys(models) as ModelKey[]).map((key) => (
                    <button key={key} type="button" onClick={() => setModel(key)} className={`tf-focus tf-mono border px-4 py-3 text-[10px] transition-colors ${model === key ? "border-[#e85b2a] bg-[#e85b2a] text-white" : "border-[#2b2925]/25 text-[#5f5b53] hover:border-[#e85b2a]"}`} aria-pressed={model === key}>{key.toUpperCase()} platform</button>
                  ))}
                </div>
              </div>
              <div className="relative min-h-[510px] overflow-hidden bg-[#2b2e2a] sm:min-h-[580px]">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#1f221f]/90 via-[#1f221f]/10 to-transparent" />
                <div className="absolute left-6 top-6 flex items-center gap-2 text-white/75 sm:left-8 sm:top-8"><Radio size={13} className="text-[#88bc62]" /><span className="tf-mono text-[10px]">Live platform study</span></div>
                <div className="absolute inset-x-6 bottom-6 sm:inset-x-8 sm:bottom-8">
                  <div className="flex items-end justify-between gap-5">
                    <div>
                      <div className="tf-mono text-[10px] text-[#f6b27c]">{currentModel.eyebrow}</div>
          ...
          <div className="tf-container">
            <div className="grid gap-12 lg:grid-cols-[.64fr_1.36fr] lg:gap-24">
              <div>
                ...
                <h2 className="mt-7 max-w-[430px] text-4xl font-medium leading-[.98] tracking-[-.055em] sm:text-6xl">Change the tool.<br /><span className="text-[#f6b27c]">Keep the rhythm.</span></h2>
        ...
        <section id="field-notes" className="tf-surface py-20 sm:py-28">
          <div className="tf-container">
            <div className="grid items-end gap-10 lg:grid-cols-[.8fr_1.2fr]">
              ...
              <p className="max-w-[430px] text-base leading-7 text-[#5f5b53]">Uneven ground. Tight rows. A weather window that starts now. Good automation doesn’t ask the farm to become something else.</p>
            </div>
            <div className="relative overflow-hidden bg-[#252724] p-3 sm:p-4">
              ...
              <div className="pointer-events-none absolute left-8 top-8 flex items-center gap-3 text-white/75"><span className="h-2 w-2 rounded-full bg-[#e85b2a]" /><span className="tf-mono text-[10px]">Short film / work in motion</span></div>
            </div>
            <div className="relative mt-12 overflow-hidden bg-[#393c35]">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#252724]/75 via-transparent to-transparent" />
              <div className="absolute left-6 top-6 flex items-center gap-3 text-white/70 sm:left-8 sm:top-8"><span className="h-2 w-2 rounded-full bg-[#88bc62]" /><span className="tf-mono text-[10px]">Observed / open field trial</span></div>
              <div className="absolute bottom-6 left-6 max-w-[320px] sm:bottom-8 sm:left-8"><div className="tf-mono text-[10px] text-[#f6b27c]">Field note 014</div><p className="mt-3 text-2xl leading-tight tracking-[-.04em] text-white sm:text-3xl">“The best system is the one the season can trust.”</p></div>
            </div>
            <div className="mt-10 grid border-y border-[#2b2925]/15 sm:grid-cols-3">
              {[{ metric: "01", label: "Operator-first", copy: "Controls that make sense before the second cup of tea." }, { metric: "02", label: "Serviceable", copy: "Built to be understood, maintained, and improved in the field." }, { metric: "03", label: "Measured", copy: "We report the work done—not just the technology used." }].map((item) => <div key={item.metric} className="border-b border-[#2b2925]/15 py-6 last:border-0 sm:border-b-0 sm:border-r sm:px-7 sm:first:pl-0 sm:last:border-0"><div className="tf-mono text-[10px] text-[#e85b2a]">{item.metric}</div><h3 className="mt-3 text-lg font-medium">{item.label}</h3><p className="mt-2 max-w-[240px] text-sm leading-6 text-[#6a655b]">{item.copy}</p></div>)}
            </div>
          </div>
        </section>
        ...
        <section id="demo" className="relative overflow-hidden bg-[#e85b2a] py-20 text-white sm:py-28">
          ...
          <div className="tf-container relative"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-24"><div><SectionKicker number="06" label="Make it practical" light /><h2 className="mt-7 max-w-[480px] text-4xl font-medium leading-[.96] tracking-[-.055em] sm:text-6xl">Bring us the row you actually run.</h2><p className="mt-6 max-w-[390px] text-base leading-7 text-white/80">Tell us what is slowing the season down. We’ll come back with a platform, a tool, and a realistic next step.</p></div><div className="bg-[#f1ede6] p-6 text-[#2b2925] sm:p-8">{submitted ? <div className="flex min-h-[320px] flex-col justify-center"><div className="flex h-12 w-12 items-center justify-center bg-[#2b2925] text-[#a6d486]"><Check size={22} /></div><h3 className="mt-6 text-3xl font-medium tracking-[-.05em]">We have your note.</h3><p className="mt-3 max-w-[340px] text-sm leading-6 text-[#6a655b]">A TerraForge field specialist will follow up shortly to understand your crop, rows, and timing.</p><button type="button" onClick={() => setSubmitted(false)} className="tf-btn tf-btn-outline mt-7 w-fit">Send another note</button></div> : <form onSubmit={handleDemoSubmit} className="space-y-5"><div className="grid gap-5 sm:grid-cols-2"><label><span className="tf-mono text-[9px] text-[#777267]">Your name</span><input required type="text" placeholder="Name" className="tf-focus mt-2 w-full border-b border-[#2b2925]/25 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#8d887e] focus:border-[#e85b2a]" /></label><label><span className="tf-mono text-[9px] text-[#777267]">Phone or email</span><input required type="text" placeholder="How should we reach you?" className="tf-focus mt-2 w-full border-b border-[#2b2925]/25 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#8d887e] focus:border-[#e85b2a]" /></label></div><label className="block"><span className="tf-mono text-[9px] text-[#777267]">What are you growing?</span><input required type="text" placeholder="Crop, acreage, or the job to solve" className="tf-focus mt-2 w-full border-b border-[#2b2925]/25 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#8d887e] focus:border-[#e85b2a]" /></label><label className="block"><span className="tf-mono text-[9px] text-[#777267]">A few words on the season</span><textarea rows={3} placeholder="Where is the pressure showing up?" className="tf-focus mt-2 w-full resize-none border-b border-[#2b2925]/25 bg-transparent px-0 py-3 text-sm outline-none placeholder:text-[#8d887e] focus:border-[#e85b2a]" /></label><button type="submit" className="tf-btn tf-btn-primary mt-2">Request a field conversation <MoveUpRight size={15} /></button></form>}</div></div></div>
        </section>
      </main>

      <footer className="tf-dark border-t border-white/10 py-10 sm:py-14">
        <div className="tf-container"><div className="flex flex-col justify-between gap-8 sm:flex-row sm:items-start"><div><AppMark dark /><p className="mt-5 max-w-[260px] text-sm leading-6 text-white/45">Field-ready autonomy for the next pass, and the one after that.</p></div><div className="grid grid-cols-2 gap-x-12 gap-y-3 sm:gap-x-16"><div className="tf-mono text-[10px] text-white/35">Explore</div><div className="tf-mono text-[10px] text-white/35">Connect</div><a href="#machine" className="text-sm text-white/70 hover:text-[#f6b27c]">The machine</a><a href="mailto:hello@terraforge.example" className="text-sm text-white/70 hover:text-[#f6b27c]">Email the team</a><a href="#attachments" className="text-sm text-white/70 hover:text-[#f6b27c]">Attachments</a><a href="#demo" className="text-sm text-white/70 hover:text-[#f6b27c]">Book a field note</a><a href="#faq" className="text-sm text-white/70 hover:text-[#f6b27c]">Questions</a><span className="text-sm text-white/35">© 2026 TerraForge</span></div></div><div className="mt-10 flex flex-col justify-between gap-3 border-t border-white/10 pt-5 text-[10px] text-white/35 sm:flex-row"><span className="tf-mono">Made for real rows</span><span>Autonomy, with both feet on the ground.</span></div></div>
      </footer>

      <a href="https://wa.me/919154153925" target="_blank" rel="noreferrer" className="tf-focus fixed bottom-5 right-5 z-40 flex h-12 w-12 items-center justify-center rounded-full bg-[#88bc62] text-[#1f221f] shadow-[0_10px_30px_rgba(31,34,31,.22)] transition-transform hover:scale-105" aria-label="Chat with TerraForge on WhatsApp"><span className="text-lg font-semibold">W</span></a>
    </div>
  );
}
